/**
 * @file
 * This file tests that routing individual writes through a mongos does not impose an overhead greater
 ...
 *
 * The workload in this case consists solely of single inserts
 ...
 * ### *Keywords*
 *
 * @module workloads/mongos_insert_single
 */
/*global
...
// Full namespace is 'mongos_insert_single.test'
var db_name = "mongos_insert_single";
...
var testInsertSingles = function(target, threads, use_sharding) {
...
var runTest = function() {
    if (!sharded()) {
        print("Workload 'mongos_insert_single' must be run in a sharded environment.");
    }
    else {
        ...
        for (var i = 0; i < thread_levels.length; i++) {
            ...
            setup(d);
            var result_direct_shard = testInsertSingles(findPrimaryShard(db_name), thread_count, false);
            teardown(d);
            setup(d);
            var result_passthrough = testInsertSingles(server, thread_count, false);
            teardown(d);
            setup(d);
            var result_sharded = testInsertSingles(server, thread_count, true);
            teardown(d);
            ...
            reportThroughput("mongos_insert_single_passthrough_ratio", passthrough_ratio, passthrough_result, is_trial_run);
            reportThroughput("mongos_insert_single_sharded_ratio", sharded_ratio, sharded_result, is_trial_run);
            reportThroughput("mongos_insert_single_passthrough_raw", result_passthrough["totalOps/s"], {nThread: thread_count}, is_trial_run);
            reportThroughput("mongos_insert_single_sharded_raw", result_sharded["totalOps/s"], {nThread: thread_count}, is_trial_run);
            reportThroughput("mongos_insert_single_direct_shard", result_direct_shard["totalOps/s"], {nThread: thread_count}, is_trial_run);
        }
    }
};
